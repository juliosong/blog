...
There are probably many methods we can use to modernize Classical Chinese teaching. Below I list three ways based on my own experience.

### 1. Introduce relatable materials
Textbooks only include carefully edited classical texts by the best authors, but it'd be great if students can see some materials that are less elevated (or totally quotidian) but more relatable as well, just like the Dunhuang doodles. Relatable materials can give modern students an idea as to how ordinary ancient people had used their language, which can in turn help them approach classical texts from a more "contemporary" perspective.
...
Note that when considering registers we must jump out of the Classical Chinese box (since "classical" itself is already a register to some extent) and view ancient Chinese from a broader perspective. As far as I'm concerned, current textbooks used in Chinese schools and universities normally cover the following ancient Chinese registers:
- literary (prose, poetry, etc.)
- (semi)colloquial (e.g., dialogs)
- (semi)factual (mostly historical records)
- fiction (e.g., plays, novels)

Registers that are rarely (if ever) seen in textbooks include:
...
I've seen many complaints about this function of *zhī*, both in real life and online, because "canceling independence of a clause" as an explanation doesn't really make much sense unless you already know what it means. That said, the English translations clearly show that this *zhī* is still used as a [genitive](https://en.wikipedia.org/wiki/Genitive_case) marker, as reflected in the underlined *of* in (1a) and the *'s* in (1b), so *zhī* is **not** the element that cancels clausal independence but rather a **result** or reflection of this "canceling," which in general linguistic terms is just clausal nominalization. Since high school students are already very familiar with nominalized clauses from English learning, I think it'd prevent unnecessary confusion if Chinese teachers could simply adopt this more general explanation rather than keep using the Chinese-specific term.
...
## Final thoughts
In this two-part post I've written down some of my thoughts on how we can learn and teach Classical Chinese more efficiently and enjoyably. Now that we have all the fancy new tools that our ancestors could only dream of, we should really make the most of them. And language learning is just a tiny corner in a gigantic space where they can make a difference. Human language has a timeless side, so one may well keep writing articles or poems in ancient Chinese in the 21st century, as long as they are equipped with the essential linguistic skills. Human language also has a universal side, so one may well acquire Classical Chinese via a general linguistic route. I wish there will be more resources exploring both directions in the future---maybe with open access. 😁
